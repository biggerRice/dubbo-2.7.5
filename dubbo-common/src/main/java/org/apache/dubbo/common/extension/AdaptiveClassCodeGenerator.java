...
public class AdaptiveClassCodeGenerator {
    ...
    private String generateMethodContent(Method method) {
        ...
        if (adaptiveAnnotation == null) {
            ...
            if (urlTypeIndex != -1) {
                // Null Point check
                // 以及为 URL 类型参数生成赋值代码，即 URL url = arg1 或 arg2，或 argN
            ...
            //将驼峰表示转为小字母.的形式如下
            //createOrderCheck 转为create.order.check
            String[] value = getMethodAdaptiveValue(adaptiveAnnotation);

            // 判断参数列表中是否含有Invocation类型参数
            // 通过判断参数名称是否等于 com.alibaba.dubbo.rpc.Invocation
    ...
    private String generateUrlAssignmentIndirectly(Method method) {
        ...
        for (int i = 0; i < pts.length; ++i) {
            // 获取某一类型参数的全部方法
            // 例如Protocol 接口的export(Invoker<T> invoker)方法
            // 此时的方法列表为invoker参数（也是接口的，也可能是数据实体）方法列表
