...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
     ...
     * 判断是否是本地调用 依次验证参数injvm url scope 三个参数
     */
    protected boolean shouldJvmRefer(Map<String, String> map) {
        URL tmpUrl = new URL("temp", "localhost", 0, map);
        boolean isJvmRefer;
        if (isInjvm() == null) {
            // url 配置被指定 表示直连服务提供者，则不做本地引用
    ...
    /**
     * 点对点直连服务提供者时，服务多可以配置在dubbo-resolve.properties文件里
     * 解析dubbo-resolve.properties文件到Properties
     * 解析interfaceName对应直连地址到url
     */
